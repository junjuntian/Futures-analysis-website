"""生猪(LH)席位能力验证的共用装载与口径。

与黄金那套(aulib/smart_money)刻意分开写,因为**生猪的合约不能当成一个东西**:
各合约相对主力偏离平均 4~12%、最大 49%(黄金不到 1%),把跨合约手数相加或者
用一条复权连续价代表整个品种,算出来的成本与收益都是错的。

所以这里一律**逐合约**:席位持仓按 (member, contract, date) 组织,收益按各合约
自己的结算价算。运营者 2026-08-19 拍板的口径也是"逐合约独立出信号"。
"""
...
SEAT_SOURCE_RANK = {"dce_official": 0, "akshare_v1": 1, "eastmoney_seats_v1": 2, "sanhe": 3}
PRICE_SOURCE_RANK = {"dce_official": 0, "akshare_v1": 1, "eastmoney_seats_v1": 2, "sina_v1": 3}

MULTIPLIER = 16.0  # LH 合约点值,与库里 product_instrument_scope 一致
...
def load_price() -> pd.DataFrame:
    ...
    但口径统一没有坏处。
    """
    df = pd.read_csv(DATA / "lh_price.csv.gz", parse_dates=["trade_date"])
...
def load_seat() -> pd.DataFrame:
    ...
    """
    df = pd.read_csv(DATA / "lh_seat.csv.gz", parse_dates=["trade_date"])
