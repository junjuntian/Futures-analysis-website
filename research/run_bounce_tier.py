...
`detect_events()` 抽成函数:移动止盈实验(PLAN_BOUNCE_TRAIL_v1)复用同一套事件检测,只换出场。
"""
import pathlib
...
def detect_events():
    """进场事件 + 两种出场(持到期 / 半 move 择时)。每个事件带上价差路径与下标,供别的出场复用。"""
    ...
    for (c1, c2), g in df.groupby(["contract_1", "contract_2"], sort=False):
        ...
        for i in range(n - 1):
            ...
            events.append(dict(c1=c1, c2=c2, inst=i1, date=d, side=side, grp=grp, n=int(nn), hit=int(hit),
                               ...
                               i=i, j_end=j_end, dirn=dirn, entry=entry, width=width, path=spread,
...
def main():
    ...
    ev.drop(columns=["path"]).to_csv(DATA / "bounce_tier_events.csv", index=False)
