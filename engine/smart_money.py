...
规则(2026-08-11 运营者拍板定稿):
  ...
  进场   = 触发席位加权建仓成本 ±5 元区间,信号后 10 个交易日内触及则成交,
           否则放弃;成本不可得时走次日开盘市价
  卖点   = 七家连续 7 个交易日无有效事件(收盘确认,次日开盘卖)/ 进场价 -4% 盘中止损
"""
...
RULES = {
    ...
    # 格林三名归一(2026-08-19):郑商所写「格林大华期货」、上期所大商所写
    # 「格林大华」,「格林期货」是 2014-03-27 前的旧名。格林大华在 AU/AG 上有仓
    # (AU 8569 行、AG 4098 行),不归一引擎会把一家当两家。
    # **「大华期货」不许加**:它与「格林期货」同日同合约并存 266 次,是 2013 年
    # 被吸收合并的另一家公司。
    ...
    # 7 日更贴合实际节奏)。这是交易判断,不是回测结论——数据上两者的取舍是:
    #   全样本资金归一 7 日 +338.7% vs 10 日 +289.3%(+49.4)
    #   但逐年只赢 7/12,且优势高度集中在 2024(+40.2)与 2026(+41.3)两年
    #   单笔均值反而更低(1.88% vs 2.35%),笔数 180 vs 125(+44%)
    #   单笔判断更差的实例:AG 2024-01-18 那笔 10 日拿满 130 天 +35.96%,
    #   7 日 7 天就走只有 +0.27%;7 日靠腾出仓位多做几笔把它补回来
    # 严格 OOS 下 A 族(改消退天数)整体为 -0.492/笔,故此改动无回测背书。
    # 回退:把 7 改回 10 即可,其余参数不受影响。
    "stop_loss": 0.04, "fade_days": 7,
...
class MarketEngine:
    ...
    def replay(self, suppress: list[tuple] | None = None) -> list[Trade]:
        ...
        #
        # 趋势态闸(DEC-058,2026-08-16 运营者采纳):收盘在 MA200 下方不做中继。
        # 中继的本质是"卖出后在趋势里接回",牛市结构不在时它是慢性失血——
        # 2008 起全量回测:AU 2011-18 熊震段中继 -11.5% 经此闸收到 -3.0%,
        # 2019-26 牛市段 +138.2%→+140.4% 零伤害,AG 同向改善(跨市场一致)。
        # 备选全部更差:"MA200 且向上"双确认 -8.9%/段合计还砍掉好单、12 月动量
        # 次优、席位净仓分位(纯席位方案)熊市不止血牛市倒扣 20 个点。
        # **只闸中继**:首进场三条件、重挫共振均不受影响。
        _ma = self.cont["adj_close"].rolling(RULES["relay_trend_ma"]).mean()
        trend_ok = (self.cont["adj_close"] > _ma).reindex(self.dates).fillna(False)
        relay_mask = ((self.score >= self.theta) & (self.theta > 0)
                      & (self.dates >= d0) & trend_ok)
