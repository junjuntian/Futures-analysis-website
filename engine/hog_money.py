...
def build_payload(sig: pd.DataFrame, mkt: pd.DataFrame, seat: pd.DataFrame,
    ...
    return {
        ...
        "bounce_long": ({
            ...
            "note": f"机构席位组净空、且本轮已卸掉 ≥{RULES['long_unload_min']:.0%}。"
                    "原验证(滚动席位组、门槛 50%)窗口内 20 日价差 +107 元/吨、涨 53%,窗口外 −123、37%;"
                    "**2026-08-23 改固定 5 家席位后复验,窗口内 20 日 −177 元/吨、涨 29%,已不优于无条件"
                    "(REPORT_LH_SPREAD_SIGNAL_v1 勘误)——这块背景目前没有数据支撑,待重验或下线。**"
                    "背景,不是进场信号。",
