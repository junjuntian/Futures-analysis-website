...
VARIETIES = {
    ...
    "FU": {
        ...
        # 选人本身站得住:四家全样本盯市盈亏都是正的(东证 +5.67 亿、东吴 +1.69、
        # 中金财富 +0.96、国金 +0.36),两家散户都是亏的(东方财富 −10.22、中银 −7.23)
        # —— 赚钱方与亏钱方分得很清,反向取用的前提成立。
        # **但这套「机构合计流向 + 散户共振」的判据在燃油上不成立**,见 backtest 与 notice。
        "fixed_members": ["东证期货", "中金财富", "国金期货", "东吴期货"],
        "fixed_since": "2026-09-07",
        "retail_seed": ["东方财富", "中银期货"],
        "retail_panel": ["东方财富", "中银期货"],
        "notice": (
            "**这台引擎的回测是亏的,别照着下单。** 全样本(2018-08 起)142 笔 "
            "**−38.8% / 夏普 −0.08 / 回撤 −82.7% / 胜率 38.7%**,"
            "同期主力买入持有 **+63.9%** —— "
            "它跑输的不是一点点,是方向就不对。逐年 2022 −53%、2023 −22%、2024 −23%、"
            "2025 −9%,只有 2026 +69%。"
            "**问题不在选谁**:换成扫描里最好的银河期货也只是 −30.9%,"
            "连不用固定名单、按年滚动重选也只有 −20.0%(全都跑输买入持有)。"
            "**四家席位本身是赚钱的、两家散户本身是亏钱的**(见配置注释里的盯市盈亏),"
            "所以不是跟错了人,是**这套判据在燃油上没有预测力**。"
            "拆开看:做空腿 73 笔合计 −39.7%,做多腿 70 笔 +48.5%,"
            "而 143 笔里有 103 笔是被「散户反向」轰出来的 —— 出场太碎。"
            "**页面留着是为了看席位持仓与流向本身,不是为了拿这条曲线交易。**"
        ),
        "out": "fu_signals.json",
        "backtest": "142 笔 −38.8%/胜率 38.7%/回撤 −82.7%/夏普 −0.08"
                    "(2018-08 起,基准 +63.9% —— **跑输基准 110 个点**)。"
                    "席位为运营者 2026-09-07 点名的固定四家,**不是回测选出来的**;"
                    "`fixed_members` 另自带前视(DEC-214 实测 +40pp)。"
                    "同名单换成本进场更差(163 笔 −79.6%/−0.45);"
                    "只看四家都在的区间(中金财富 2022-10 才首见)是 67 笔 +8.7%/0.22/−59.4%。"
                    "候选替换实测:国金→银河 125 笔 −30.9%/0.00;国金→瑞达 −51.9%;"
                    "对照(不用固定名单、按年滚动)123 笔 −20.0%/0.06。"
                    "(研究脚本按 09-04 快照跑的是 143 笔 −45.8%,差在末笔未平仓;"
