...
VARIETIES = {
    ...
    "FG": {
        ...
        # ②弱年、回撤这类**会随规则变的数一律不写死在这里** —— 上面这段原先挂着
        # 满仓年代的「2016 −34%、2022-23 连亏两年、回撤 −46%」,DEC-234 上了分数仓位
        # 之后实际是 2016 −2.7%、2022/2023 +1.5%/+5.3%、回撤 −26.7%,而文案原地不动
        # (运营者 2026-09-06 当场问「是不是搞错了」)。**页面卡上就渲染着实时的
        # 累计/夏普/回撤/逐年**,要看数看那儿;守卫测试禁止 note 里再出现「年份+百分比」。
        "follow_seat": {
            "member": "永安期货",
            ...
            "note": ("第二引擎,与主引擎并列、各管各的仓位(实测两者日收益相关 +0.31,"
                     "50/50 组合夏普高于任一台单跑,回撤反而更浅)。规则:永安在"
                     "当日主力的可见净持仓方向,翻转→次日开盘反手,约每月一次多点;"
                     "**没有止损、没有持有上限、不看散户也不看主引擎**,永远在场,只翻不停。"
                     "**丑话**:永安流向 IC 不显著(t=1.72),信息在仓位水平,缺独立"
                     "旁证,14 年安慰剂 p=0.000(过五选一校正)是主要依据。"
                     "弱年与回撤请看上面这张卡的**逐年与回放**(实时算的)—— "
                     "这段话里不写死数字,写死过一次就在 DEC-234 上分数仓位之后当场过期。"
                     "验收 REPORT_FGSA_MODEL_v1。"),
        },
        "out": "fg_signals.json",
    ...
    "FU": {
        ...
        # 席位 2018-07-25 起(新 FU 380CST 2018-07-16 上市后第 8 个交易日交易所才发排名),
        # 留一周预热。行情更早(2018-01),但那是**旧 FU 合约的休眠期**,近乎零成交,不是样本。
        "replay_start": "2018-08-01",
        "long_enabled": True, "long_needs_dip": False,
        # **席位是运营者点名的(2026-09-07),不是回测选出来的**。
        # 选人本身站得住:全样本盯市盈亏都是正的(东证 +5.67 亿、银河 +8.20、
        # 东吴 +1.69、中金财富 +0.96),两家散户都是亏的(东方财富 −10.22、中银 −7.23)
        # —— 赚钱方与亏钱方分得很清,反向取用的前提成立。
        # **但这套「机构合计流向 + 散户共振」的判据在燃油上不成立**,见 backtest 与 notice。
        #
        # **国金 → 银河(同日运营者拍板换的)**。换的理由不是国金亏钱(它赚 +0.36 亿),
        # 是**它几乎不在这个品种里**:上榜率仅 16%、中位净持仓 **69 手**
        # (DEC-195 用同一条判据否掉过招商 651 手、中信建投 171 手,它比那两个还小
        # 一个量级)、近两年择时收益 +0.03 亿、**2026-08-19 之后就不在榜上**。
        # 银河:近两年 +1.82 亿(88 家池子第 1)、近一年 +1.57、全样本 +14.82、
        # 上榜率 **100%**、中位持仓 2,719 手、最赚 5 天占比 41%(与东证 38%、瑞达 43% 同档)。
        # **丑话**:银河是从 88 家里挑出来的。噪音判据说这张榜有信息
        # (榜首 +1.82 亿 vs 纯噪音取最大值的期望 +1.47 亿,第 89 百分位 ——
        # 对比焦煤那次第 22 百分位、连噪音都不如),**但 89% ≈ 单侧 p 0.11 不到 0.05**:
        # 「银河比国金好」可信(差 60 倍),「银河是最好的那一家」未被证明。
        # **换完引擎照样是亏的**(见下面的 backtest)—— 换人换的是名单的合理性,
        # 不是把这条曲线救回来。
        "fixed_members": ["东证期货", "中金财富", "银河期货", "东吴期货"],
        "fixed_since": "2026-09-07",
        "retail_seed": ["东方财富", "中银期货"],
        "retail_panel": ["东方财富", "中银期货"],
        "notice": (
            "**这台引擎的回测是亏的,别照着下单。** 全样本(2018-08 起)124 笔 "
            "**−23.4% / 夏普 0.00 / 回撤 −80.0% / 胜率 38.7%**,"
            "同期主力买入持有 **+63.9%** —— 它跑输的不是一点点,是方向就不对。"
            "8 年里有 5 年是亏的。"
            "**问题不在选谁**:国金换成银河把它从 −38.8% 抬到 −23.4%,但仍是亏;"
            "换瑞达 −51.9%,连不用固定名单、按年滚动重选也只有 −20.0%(全都跑输买入持有)。"
            "**问题也不在进场规则**:换成纯碱那套(成本进场)−50.8%、"
            "换成玻璃那套(成本+轮龄≥2+仍在加仓)−33.1%、玻璃全套(再加水位≤60%)−50.1%,"
            "**一格都没比现在好**(2026-09-07 实测,运营者点名要试的)。"
            "**四家席位本身是赚钱的、两家散户本身是亏钱的**(见配置注释里的盯市盈亏),"
            "所以不是跟错了人,是**这套判据在燃油上没有预测力**。"
            "拆开看:做空腿合计亏、做多腿合计赚,"
            "而 124 笔里有 90 笔是被「散户反向」轰出来的 —— 出场太碎。"
            "**页面留着是为了看席位持仓与流向本身,不是为了拿这条曲线交易。**"
        ),
        "out": "fu_signals.json",
        "backtest": "124 笔 −23.4%/胜率 38.7%/回撤 −80.0%/夏普 0.00"
                    "(2018-08 起,基准 +63.9% —— **跑输基准 87 个点**)。"
                    "席位为运营者 2026-09-07 点名的固定四家(同日把国金换成银河),"
                    "**不是回测选出来的**;`fixed_members` 另自带前视(DEC-214 实测 +40pp)。"
                    "只看四家都在的区间(中金财富 2022-10 才首见)是 61 笔 +21.4%/0.31/−56.4%。"
                    "**换名单**:原名单(国金)−45.8%/−0.08、→银河 −30.9%/0.00、"
                    "→瑞达 −51.9%/−0.12、对照(不用固定名单按年滚动)−20.0%/0.06。"
                    "**换进场规则(运营者点名要试玻纯那套,2026-09-07 实测)**:"
                    "纯碱式(成本裸)138 笔 −50.8%/−0.19;"
                    "玻璃式(成本+轮龄≥2+仍在加仓)82 笔 −33.1%/−0.12(回撤 −49.8% 是浅了,"
                    "但那是笔数从 125 砍到 82 换来的,累计与夏普都更差);"
                    "玻璃全套(再加水位≤60%)68 笔 −50.1%/−0.32。"
                    "**翻向门在燃油上一次都没触发**(加与不加逐位相同)。"
                    "**六种名单 × 五种进场,没有一格跑赢买入持有。**"
                    "**跟随东证的手数也不行(2026-09-07 运营者点名要试)**:"
                    "满仓跟方向 −83.8%/−0.40/−89.2%,跟手数(200 日水位)−39.2%/−0.32、"
                    "(400 日)−36.9%/−0.34。G1「胜过等效缩仓」形式上过了(−0.32 vs −0.42),"
                    "**但两边都是负的,在负值上比谁更不差没有意义**;"
                    "安慰剂 **p=0.795**(随机平移的序列有八成能做得一样好),两个半样本也都是负的。"
                    "根因是**东证在燃油上 8 年翻了 469 次 ≈ 每 4 个交易日一翻**"
                    "(玻璃跟永安约 15 次/年、焦煤跟华泰约 22 次/年,东证在燃油上是 59 次/年)——"
                    "`JM_SINGLE_SEAT` 当年就是这么死的(「东证 2 天一翻,T+1 跟不上」)。"
                    "成交时点三格更直白:T 结算(上界)+0.45、T+1 开盘(现行)−0.32、"
                    "T+1 收盘(下界)−0.41 —— **信息全在拿不到的那段窗口里**(同 DEC-238)。"
                    "(研究脚本跑的是 125 笔 −30.9%,差在末笔未平仓;"
                    "**页面上的数字以本引擎自己算的 stats 为准**)。"
                    "验收见 REPORT_FU_ENGINE_v1",
    },
    "AP": {
        "name": "苹果 AP", "unit": "元/吨", "multiplier": 10.0,
        # 2017-12-22 上市,席位同日起;留一个多月预热。
        "replay_start": "2018-02-01",
        # 苹果 2018 以来有明确的上下行段,只做空会对上行装瞎(与 FU 同理)。
        "long_enabled": True, "long_needs_dip": False,
        # **最小配置,不挑参数**(PLAN_AP_ENGINE_v1)。运营者 2026-09-22 只说
        # 「把苹果加进去」,**没有点名席位**,所以席位组走平台默认的按年滚动重选,
        # **不配 fixed_members** —— 那个通道自带前视(DEC-214 实测 +40pp),
        # 没人点名就不该用。散户走平台默认选法,不配 retail_seed。
        # sizing / flip_gate / 第二引擎 / 换月接力一律不配,那些都是逐品种验过才开的。
        "out": "ap_signals.json",
        # 数字取自**生产**(2026-09-22 那天的 ap_signals.json),不是本地研究快照 ——
        # 本地 research/data 落后生产一两天,拿它写这段会和页面上的 stats 对不上
        # (LATEST 里「研究快照的末日要先看一眼」那条教训)。这段每天都会略微漂,
        # 与其它品种同例,所以写明**截至日**。
        "backtest": "截至 2026-09-22:126 笔 复利净 +89.8%/夏普 0.47/回撤 −31.5%"
                    "(2019-03 起,滚动重选要先攒一年数据;同期主力买入持有 −6.2%、"
                    "恒定满仓做空 −45.0% —— **两个基准都跑赢了**)。"
                    "**两腿都是赚的**(做多 +51.8%、做空 +43.4%;燃油是做空腿亏)。"
                    "**丑话**:单笔均值 t≈1.8(<2),统计上还没证明成立;"
                    "**8 年里 4 年亏,而且最近两年(2025、2026)都是亏的**;"
                    "约八成的笔数被「散户反向」轰出场,出场太碎;"
                    ...
                    "上面的数字已经是加了门槛之后的(同一份快照上,加之前是 +76.6%/0.43/−31.1%)。"
