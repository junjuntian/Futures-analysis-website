...
def build_payload(sig: pd.DataFrame, mkt: pd.DataFrame, seat: pd.DataFrame,
    ...
    return {
        ...
        "bounce_long": ({
            ...
            "note": "机构席位组净空时,按「本轮已卸掉多少 × 价差当年位置」给背景(DEC-128,2026 回测,"
                    "固定 5 家席位,生猪跨月 16 组 893 组合日):价差低位且只卸掉 10~20% → 之后 20 日 "
                    "+174 元/吨、涨 65%(反弹参考区间);低位但已卸 >20% → −89~−157(反弹多半走完);"
                    "价差高位不论卸多少 → −289~−465(别追)。**只有 2026 一年约 5 个独立波段,"
                    "不是全样本验证;磨底年过去要回头重验。** 原版「卸够 50% 即窗口」(滚动组)在固定名单下"
                    "已不成立(REPORT_LH_SPREAD_SIGNAL_v1 勘误)。背景,不是进场信号。",
