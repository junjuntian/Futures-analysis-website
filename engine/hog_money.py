...
VARIETIES = {
    ...
    "I": {
        ...
        # **上方那行「观察中/无持仓」是主引擎说的,而主引擎在这个品种上没过闸。**
        # 真正过闸的是下面第二引擎那张卡 —— 两者可能同时显示「无持仓」和「做空中」,
        # 那不是矛盾,是两台引擎各说各的。这一栏必须把「该看哪一张」写明白。
        "backtest": "**主引擎(通用共振口径)在铁矿石上没过闸,而且是亏的** —— "
                    "八格全扫最好的一格单笔 t 仅 +1.53、逐年 2/4、基本靠 2026 一年"
                    "(DEC-183,REPORT_I_ENGINE_v1);上方风险警示框那四条是实算的,"
                    "不是显示故障。**「今日信号」这一格不该照着下单。**"
                    "**这个品种真正过闸的是下面那张「第二引擎 · 跟永安」** —— "
                    "与焦煤跟华泰、玻璃跟永安同一口径(候选池=席位组五家),"
                    "五选一校正后 p=0.010/0.018 过、走前 +9.7%(11 次中 9 次挑中)、"
                    "单席位安慰剂 p=0.002。**要跟就跟那一张**,丑话印在它自己卡上"
                    "(DEC-184,REPORT_I_FAIR_v1)",
    },
    "SA": {
