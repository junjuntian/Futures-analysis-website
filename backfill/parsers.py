...
WANT = {"AP", "FG", "SA", "AU", "AG", "I", "IH", "SC", "JM", "JD", "LH", "FU", "MA"}
...
EXCHANGE_BY_VARIETY = {
    ...
    "IH": "CFFEX",
    "SC": "INE",
    # 燃料油(2026-09-07 运营者点名)。**上期所,不是能源中心** —— 低硫燃料油 LU
    # 才是能源中心的,两者中文名只差两个字,标错交易所就会写出两份互不去重的行。
    "FU": "SHFE",
    # 甲醇(2026-09-15 运营者点名)。郑商所。
    #
    # **只收 MA,绝不收 ME**:郑商所 2014 年中起把甲醇从 `ME` 换成 `MA`,
    # **同时把合约单位从 50 吨/手改成 10 吨/手**(2015-06 前后 ME 全部退完)。
    # 两个代码是两个不同的合约,点值差 5 倍 —— 合到一个品种里,盈亏会整段算错。
    # `WANT` 里没有 "ME",所以 ME 的行会被 `wanted_from` 自然滤掉,
    # 这条注释是为了**别有人日后"顺手把历史补全"**。
    "MA": "CZCE",
}
VARIETY_BY_NAME = {
    "沪金": "AU",
    ...
    "原油": "SC",
    # 燃料油:交易所、东财、三禾三家叫法一致,不像沪金/黄金那样分裂。
    "燃料油": "FU",
    # 甲醇:郑商所席位文件里写「品种:甲醇MA」,三禾/东财也叫「甲醇」。
    "甲醇": "MA",
}
