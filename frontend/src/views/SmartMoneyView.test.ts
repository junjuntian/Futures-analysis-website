...
describe('SmartMoneyView 历史信号分页', () => {
  ...
  it('品种按钮由 FLOW 列表渲染,加品种只改那一行', async () => {
    ...
    //
    // **上证50 与铁矿石都是有意单列的**(DEC-172 / DEC-178):
    //   · 上证50 的信号是「跟三家席位的在场方向是否一致」(IhFollow);
    //   · 铁矿石是「跟永安一家的方向」+ 手数方案(IronFollow),且**没有主引擎**。
    // 两者都不是阵营 z 分数,渲染的组件也不同。所以它们**不在 FLOW 里**,
    // 是模板里各自单独一个按钮 —— 塞进 FLOW 会让 v-for 渲染出点了会崩的标签。
    const wrapper = mount(SmartMoneyView, { global: { plugins: [ElementPlus] } })
    await flushPromises()
    const labels = wrapper.findAll('.variety').map((b) => b.text())
    expect(labels).toEqual(
      ['黄金白银', '生猪', '鸡蛋', '焦煤', '玻璃', '纯碱', '上证50', '铁矿石'])
    wrapper.unmount()
  })
