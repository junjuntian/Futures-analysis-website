...
describe('SmartMoneyView 历史信号分页', () => {
  ...
  it('品种按钮由 FLOW 列表渲染,加品种只改那一行', async () => {
    ...
    //
    // **上证50 是有意单列的**(DEC-172):它的信号是「跟三家席位的在场方向是否
    // 一致」,与阵营 z 分数不是一回事,渲染 IhFollow 而不是 HogMoney,所以
    // 它**不在 FLOW 里**,是模板里单独一个按钮 —— 塞进 FLOW 会让 v-for 渲染出
    // 一个点了会崩的标签。
    //
    // **铁矿石在 FLOW 里**(DEC-178 二版,运营者:「要做得跟焦煤生猪一样的界面」):
    // 它走与其余五个品种完全相同的那条路,初版那个单独组件已经删掉。
    const wrapper = mount(SmartMoneyView, { global: { plugins: [ElementPlus] } })
    await flushPromises()
    const labels = wrapper.findAll('.variety').map((b) => b.text())
    // **燃油在纯碱后面**(2026-09-07 运营者指定的位置)。顺序也钉住:
    // 苹果(2026-09-22)运营者没指定位置,接在燃油后面、铁矿石前面。
    // 他是按自己的看盘习惯排的,别人「顺手按字母排一下」会把它挪走。
    expect(labels).toEqual(
      ['黄金白银', '生猪', '鸡蛋', '焦煤', '玻璃', '纯碱', '燃油', '苹果', '铁矿石', '上证50'])
    wrapper.unmount()
  })
