...
/** 成本对应的方向与手数(运营者 2026-09-04:「跟品种汇总一样」)。
 *
 *  **措辞与品种汇总那排对齐**,因为这两个数本来就是同一件事:引擎是**净持仓计价**,
 *  每个合约先算出净额与净额成本,再**按净额正负分进「多单桶」或「空单桶」**;
 *  品种汇总的「多 9,890 手 均价 1055.10」就是把这一列里净多的那几行加权出来的
 *  (2026-09-04 用生产数逐行验过:5,191+815+3,884=9,890,加权 1055.10,一分不差)。
 *  原来这里写「净多 5,191 手」,和汇总那排的「多单/均价」看着像两回事,其实是同一个数。
 *
 *  **逐合约没有「多单成本」和「空单成本」两个数** —— 一个合约一个净额成本,
 *  它要么在多单桶要么在空单桶。所以这里说「多单均价」而不是「多头持仓的成本」:
 *  它对应的是净额那 5,191 手,不是毛多头 7,524 手。 */
function costNetLabel(value: string) {
  const n = Number(value)
  if (n === 0) return '净持仓 0'
  return `${n > 0 ? '多单' : '空单'}均价 · ${fmt(Math.abs(n))} 手`
}
        ...
        <table v-else class="positions">
          <thead>
            <tr>
              ...
              <th>多头持仓</th>
              <th>空头持仓</th>
              <th title="由公开持仓变化与结算价推出,不是成交均价;净持仓计价——一个合约一个成本,按净额正负归入多单或空单。品种汇总那排的多空均价就是这一列加权出来的,同一个引擎">
                成本<span class="th-sub">净额计价·归多空</span>
              </th>
            </tr>
          </thead>
          <tbody>
