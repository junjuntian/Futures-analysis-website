...
const FILES: Record<FlowCode, string> = {
  ...
  FU: 'fu_signals.json', AP: 'ap_signals.json'
}
...
interface HogPayload {
  ...
  rules: { reselect_months: number; group_k: number; enter: number; stop: number;
           ...
           /** 重仓翻向门(DEC-228)与换组不清零卸仓(DEC-230):逐品种开关。 */
           flip_gate?: boolean; unload_regroup_seed?: boolean
      ...
      <div class="cards">
        <div class="card wide">
          ...
          <ol class="rules">
            ...
            <!-- **别再把家数写死**:纯碱 2026-09-06 起是 4 家(DEC-219 剔瑞达),
                 写死的「5 家」当场就错了。一律读 data.members.length。 -->
            <li><b>信号</b>:这 {{ data.members.length }} 家在**全品种合约上的合计净持仓**,取 {{ data.signal.win }} 日变化,
            ...
            <li v-if="!isCampaign && isCost"><b>进场(成本信号,DEC-112)</b>:**机构在场 +
              ...
              加权),不是交易所真值。散户那一路仍在 —— 只管出场。<br>
              **「本轮卸仓」怎么算**:`1 − 今日合计净持仓 ÷ 本轮峰值`。
              「本轮峰值」是**本轮开始到今天为止见过的最大合计净持仓**(只往上走,
              今天创新高就等于今天,于是卸仓 = 0%)。**本轮**在三种情况下重来:
              方向翻转 / 归零 / 换组。掉榜那天**冻结不给值** —— 掉榜是「不知道」不是「卸完了」。
              <template v-if="data.rules.unload_regroup_seed">
                **换组不清零(DEC-230)**:改用**同方向的近 9 个月高位**当新一轮的起点。
                清零本意是「别把换人读成出货」,但代价是换人当天门必然打开 ——
                2026-09-03/04 连着两次换组,就把已经卸到 54% 的那一轮清成了 0%。
              </template></span></li>
            <!-- DEC-228:这条**必须写在页面上** —— 它是个会真的挡住进场的判据,
                 而运营者第一次看策略方案时问「前面写进去的拦截方案怎么没看到」。 -->
            <li v-if="data.rules.flip_gate"><b>拦截(重仓翻向门,DEC-228)</b>:
              **某一家在主力合约上翻向**(今天的净持仓方向 ≠ 20 个交易日前)
              **且新方向仓位 ≥ 自身 200 日高位的 60%** → **不往它的反方向进场**
              (它翻多就不做空,翻空就不做多)。可能是反弹或转向,不跟。<br>
              <span class="hint">口径三处要点:①用**主力合约**净持仓,不用全品种合计
              —— 2026-08 玻璃永安在 FG2701 上净多 94,903 手(水位 71%),
              而全品种合计只有 55,371 手/39%,**主力上的九万手被其他月份的空单对冲掉了**;
              ②窗口 200 日;③翻向看的是 20 日前的方向,不是单日符号变化(会被零附近的抖动刷屏)。<br>
              **证据形态要说清**:它抓到了 2026-08 玻璃永安那次转向(连续多日),
              但**机制统计接近抛硬币** —— 触发后顺翻向者方向的命中率,玻璃 5/10/20 日
              54%/48%/**42%**、纯碱 55%/50%/49%;**玻璃回测因此掉约 6 个点**。
              这是知情选择上线,不是验证通过。</span></li>
            ...
            <li v-if="!isCampaign && isInstExit"><b>出场(机构出场,DEC-117)</b>:**机构席位组方向翻转,
              ...
              **不看散户翻向、不设持满**。它赢的方式是「机构一松手立刻走、再上手立刻跟」
              ...
              <span class="hint">{{ exitText }}</span></li>
            <!-- DEC-218:纯碱把散户那一路从出场里整条拿掉了,只剩三条纪律。
                 **这一条必须按 exit_mode 分叉**,否则页面会继续写「散户反向翻向」,
                 而引擎根本不读它 —— DEC-104 就是这么来的(文案与判据对不上)。 -->
            <li v-else-if="!isCampaign && data.rules.exit_mode === 'discipline'"><b>出场</b>:
              **只剩纪律三条** —— 硬止损 {{ (data.rules.stop * 100).toFixed(0) }}% /
              持满 {{ data.rules.max_hold }} 个交易日<template v-if="data.rules.exit_before_delivery">
              / **主力进入交割窗口前 {{ data.rules.exit_before_delivery }} 个交易日
              强制平仓**(散户纪律,这段时间也不进场)</template>。
              **散户反向不再参与出场**(DEC-218):改之前八成仓由它平掉、中位只持 4 天,
              而一轮完整趋势有三个月;整条拿掉后
              −44.8%/夏普 −0.33/回撤 −57.5% → **+118.9%/0.61/−40.9%**(满仓口径)。
              **为什么是整条拿掉而不是调阈值**:实测散户水平门与「反向连续 k 日」两个候选
              都倒在安慰剂(p=0.892 / 0.596)——起作用的是「少出场」,散户信息本身没有贡献。<br>
              <span class="hint">{{ exitText }}
              **要知道**:出场里将近一半是「临近交割」,那是被日历赶出来的、不是判断。</span></li>
            ...
            <li v-if="data.sizing"><b>仓位(分数仓位,DEC-224)</b>:**不是满仓进场**。
              ...
              全样本只吃 1pp 上下)。<br>
              <span class="hint">核心检验是「必须胜过等效缩仓」:同样把平均仓位压到
              {{ Math.round((data.sizing.strength ?? 0) * 100) }}% 一档,固定缩仓只能把夏普维持在原地,
              按水位调仓把它抬上去 —— 多出来的那一截是择时价值,不是缩仓效果。
              **弱点照写**:逐年那关没过(玻璃 9/14、纯碱 4/7),改善不是每年都有。</span></li>
