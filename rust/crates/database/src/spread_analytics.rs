...
mod tests {
    ...
    fn rust_normalisation_matches_the_sql_member_key() {
        ...
        for (raw, expected) in [
            ...
            ("申银万国", "申万期货"),
            ("申银万国（代客）", "申万期货"),
            // 格林三名归一：郑商所写「格林大华期货」、上期所大商所写「格林大华」，
            // 「格林期货」是 2014-03-27 之前的旧名。详见 MEMBER_ALIASES 上的核查记录。
            ("格林大华期货", "格林大华"),
            ("格林大华期货（代客）", "格林大华"),
            ("格林期货", "格林大华"),
            ("格林大华（代客）", "格林大华"),
            // 现用名不受别名影响。
            ("高盛期货（代客）", "高盛期货"),
            // **「大华期货」是另一家公司，归一后必须还是它自己。**
            // 它与「格林期货」同日同合约同时上榜 266 次（2008-04-29 ~ 2013-09-17），
            // 2013 年被吸收合并。名字长得像不等于是同一家——这条断言就是拦住
            // 「看着像就顺手并进去」的。
            ("大华期货", "大华期货"),
        ] {
...
///
/// 格林那三个名字是**两种成因叠在一起**（2026-08-19 查库核实）：
/// 「格林大华期货」只出现在郑商所的 AP/FG/SA，「格林大华」只出现在上期所的
/// AU/AG 与大商所的 JD/JM/LH——**品种零重叠**，纯粹是两边写全称与写简称的差别；
/// 「格林期货」是 2014-03-27 之前的旧名，交接当天两个名字的持仓**逐笔相同**
/// （AG1406 的 11603、AU1406 的 1486/906、AU1412 的 34、FG1409 的 505），
/// 且重复的那份是回榜反推行，去重时官方行优先，并进来既不会重算也不会丢。
///
/// **「大华期货」不在这张表里，也不许加进来**：它 2008-04-29 ~ 2013-09-17 与
/// 「格林期货」在同日同合约同时上榜 **266 次**——同一家不可能在一张榜上占两个
/// 名次，那是被 2013 年吸收合并掉的另一家公司。并进来会把两家的持仓叠成一家。
///
