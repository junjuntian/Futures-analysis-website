...
/// - **FU 燃料油 5%(2026-09-15 定档,运营者拍板)**:机制与 JD 同源 ——
///   **早进不受罚**。5% 档早进组中位 **+12.2**(JD 当年是 +7.4),仍然赚钱;
///   档位越深早进组越差(10% 时 +1.9、25% 时 −10.7),深回撤对它纯粹让利。
///   全样本 132~135 笔逐档单调无翻转:5% 均值 +20.8 / 中位 +17.5 / 胜率 59% /
///   MAE 14.7,10% 是 +15.5 / +6.5 / 55% / 19.5。留一法 OOS 中位 +18.7 vs
///   固定 10% 的 +8.0。
///   **丑话**:逐年一致性只有 **5/9**(与当初否掉 AP 的 5/8 同档),样本量只有
///   DEC-070 标准(每档 350~380 笔)的三分之一,且 2023~2025 三年反而是深档更好。
///   留下来靠的是「机制 + 单调性 + OOS 改善」三条,不是一致性。
///
...
mod monitor_tests {
    ...
    fn the_retreat_line_is_per_variety() {
        ...
        // 2026-09-15:FU 与 JD 同机制(早进不受罚,5% 档早进组中位仍 +12.2),
        // 逐档单调无翻转。别按「远月波动小」之类的直觉改深。
        assert_eq!(turn_retreat("FU", "FU"), 0.05);
