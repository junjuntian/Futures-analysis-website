...
- 采集渠道现状：五家交易所走各自官方 akshare 接口；DCE 官方全线 412，实际走新浪 fallback（`DEC-041`）；**东方财富已按 `DEC-043` 接入为席位专用兜底源**（`datacenter-web.eastmoney.com` 报表 `RPT_FUTU_DAILYPOSITION`，不经 akshare，排在全部官方源之后，五家通用，不承担行情与目录，INE 排除）。`1111` 里记的 `qhhqzl.eastmoney.com/...` 地址是错的。
