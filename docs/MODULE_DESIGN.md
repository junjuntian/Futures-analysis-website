...
| 图表 | `chart` | 图表模板、配置、导出任务 | `spread`、`portfolio`、`seat_analysis`、`storage` |
| 数据连接器 | `connector` | 授权站点会话、发现、提取、快照 | `storage`、`import`、`jobs` |
...
- `connector` 的提取结果必须经过 `import` 的预览与确认，不直接写正式业务表。
- `connector` 只处理白名单域名和已实现连接器；每个 Workspace/数据源使用独立 Browser Context。
...
- SQLx 仓储、本地/S3 存储、Playwright、OCR、AI Provider、时钟和加密实现。
- 适配器失败必须映射为稳定的应用错误。
...
| `DataConnector` | `validate_session`、`discover`、`extract` |
| `AiProvider` | `capabilities`、`chat`、`stream` |
| `ChartRenderer` | `render_png`、`render_svg` |
| `Clock` | `now` |
...
| `dependency_unavailable` | OCR/AI/浏览器不可用 | 可退避重试 |
| `rate_limited` | 外部站点或模型限流 | 按 `retry_after` 重试 |
| `data_quality_error` | OCR 低置信度、腿价格缺失 | 进入人工确认，不自动重试写入 |
...
- 浏览器/OCR 资源隔离已成为 API/Worker 的稳定性瓶颈。
