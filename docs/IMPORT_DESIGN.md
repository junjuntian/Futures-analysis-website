...
导入中心是 MVP 的第一业务能力。它必须做到可预览、可复核、可重试、可追溯和可回滚，并作为文件导入、网页提取和 OCR 确认后的统一数据入口。
...
网页和 OCR 输入的提取顺序固定为：官方 API、网络请求、HTML 表格、下载文件、OCR。OCR 数据必须停留在预览状态，只有人工确认并写入审计记录后才能创建正式导入批次。
